...
7. **Touch, the saga — SOLVED.** The documented protocol (I2C 0x3B) never ACKs on some units. The real story: the board shipped in **two revisions with different touch chips**. Old boards (2023 – early 2026) use the AXS15231B's integrated touch at 0x3B. Newer boards (~May 2026 on, matching LilyGo's `T-Display-S3-Long-cst3530` branch) have a separate **Hynitron CST3530** at I2C **0x58** (boot/ISP mode at 0x5A, reset on **GPIO2**, not the shared GPIO16). Talking 0x3B to a CST3530 board gets you nothing but INT pulses — hence the original blind-tap workaround. `touch_drv.h` now probes both addresses at boot and speaks the right protocol; on CST boards it also verifies the chip's resident firmware (Hynitron chips store ~40KB host-uploadable firmware; the blob from LilyGo's repo is compiled in via `cst3530_fw.h`) and uploads it if missing or stale. Result: real coordinates on both revisions — tap and swipe. The blind-tap fallback remains for anything unrecognized.
